...
with open(csv_file_name_relevant) as csv_rel:
    ...
    for row in csv_file_relevant:
        training_data.append({'class': 'relevant', 'title': row[0], 'text': row[1]})
    for i in range(5):
        test_data.append(training_data.pop())

with open(csv_file_name_not_relevant) as csv_not_rel:
    csv_file_not_relevant = csv.reader(csv_not_rel)
    for row in csv_file_not_relevant:
        training_data.append({'class': 'not_relevant', 'title': row[0], 'text': row[1]})
    for i in range(5):
        test_data.append(training_data.pop())
...
train(X, y, hidden_neurons=250, alpha=0.1, epochs=10000, dropout=False, dropout_percent=0.2)
